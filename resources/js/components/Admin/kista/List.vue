<template>
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row">
          <div class="col-sm-6">
            <h5 class="m-0 text-dark xyz">Kista List ({{this.manager_name}})</h5>
          </div><!-- /.col -->
          <div class="col-sm-6">
                  ...
                  <table class="table table-bordered table-hover table-sm m-0">
                    ...
                    <tbody class="text-center">
                      <tr v-for="(data,index) in getALLLuckyDraw" :key="data.id" :class="colorchange(data.is_active)">
                        ...
                        <td>{{data.amount}}</td>
                        <td>{{data.get_lucky_draw.name}}</td>
                        <td v-if="data.is_active == '0'">Inactive <a href="javascript:void(0)" @click.prevent="KistaStatus(data.id, data.is_active)" title="Click to Publish"><i class="nav-icon fas fa-times-circle text-danger"></i></a></td>
                        <td v-else>Active <a href="javascript:void(0)" @click.prevent="KistaStatus(data.id, data.is_active)" title="Click to Unpublish"><i class="nav-icon fas fa-check-circle text-success"></i></a></td>
                        <td>
                          {{data.date_np}} 
                        ...
                        <td>
                          ...
                          <a href="" @click.prevent="deleteKista(data.id)" class="btn btn-xs btn-outline-danger"><i class="fas fa-trash-alt" title="Click to delete"></i></a>
                        </td>
                      </tr>
                    </tbody>
                  </table>
