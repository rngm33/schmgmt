  ...
  <div>
    <div class="content-header">
      <div class="container-fluid">
        <div class="row">
          <div class="col-sm-6">
            <h5 class="m-0 text-dark xyz">Income/Expenditure List</h5>
          </div><!-- /.col -->
          <div class="col-sm-6">
          ...
          <section class="col-lg-12 connectedSortable">
            ...
            <div class="card card-info card-outline">
              <div class="card-header">
                <div class="row">
                  <div class="col-md-2">
                    <router-link :to="'/incomeexpenditure/'+this.$route.params.incomeexpenditure+'/create'" class="btn btn-primary btn-block" style="color:#fff"> Add <i class="fas fa-user-plus fa-fw"></i></router-link>
                  </div>
                  <div class="col-md-10">
                ...
                <div class="table-responsive">
                  <table class="table table-bordered table-hover table-sm m-0">
                    ...
                    <tbody class="text-center">
                      <tr v-for="(data,index) in getAllIncomeExpenditure" :key="data.id" :class="colorchange(data.is_active)">
                        <td>{{index+1}}</td>
                        <td v-if="data.type == 'Expenditure'">{{data.type}}(<span v-if="data.expenditure_type == '1'">Direct</span><span v-else>Indirect</span>)
                        </td>
  ...
  export default{
    ...
    computed:{
      getAllIncomeExpenditure(){
        this.$Progress.start()
        var avar = this.$store.getters.getIncomeExpenditure;
        if(avar.length==2)
    ...
    methods:{
      ...
      IncomeExpenseStatus(clkid, show){
          axios.get('/manager/incomeexpenditure/status/'+clkid+'/'+show)
          .then(()=>{
      ...
      deleteIncomeExpenditure(id){
        ...
        Swal.fire({
          ...
          if(isConfirm.value === true) {
            axios.delete('/manager/incomeexpenditure/'+id)
            .then((response)=>{
              Toast.fire({
                icon: 'success',
                title: 'Data Deleted successfully'
              })
              that.$store.dispatch("allIncomeExpenditure", [0,0]);
            })
            .catch((response)=>{
              Toast.fire({
                icon: 'error',
